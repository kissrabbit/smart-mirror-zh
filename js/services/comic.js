(function () {
    ...
    function ComicService($http) {
        ...
        service.getXKCD = function () {
            return $http.jsonp("http://dynamic.xkcd.com/api-0/jsonp/comic?callback=JSON_CALLBACK").then(function (response) {
                return response.data;
            });
        };

        service.initDilbert = function () {
            return $http.jsonp('http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=50&callback=JSON_CALLBACK&q=http://rss.latunyi.com/dilbert.rss').then(function (response) {
